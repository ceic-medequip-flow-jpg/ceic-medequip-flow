...
# Locate the component definition to its end
match = re.search(r'(const PendingRequestCard = \(\{ req.*?)(const InventoryView = )', content, re.MULTILINE|re.DOTALL)
if match:
    old_block = match.group(1)
    
    # We will redefine PendingRequestCard up to the end exactly as needed.
                                        ...
                                        <div className="flex-1">
                                            {isTransport ? (
                                                ...
                                                <>
                                                    <div className="flex flex-col sm:flex-row items-stretch gap-2">
                                                        <div className="relative flex-1">
                                                            ...
                                                            <input className="input pl-9 font-mono w-full" list={`tags-list-${req.id}`} value={typedTag} onChange={(e) =>
                                                                setTypedTag(e.target.value.toUpperCase())} placeholder="Digite ou selecione a TAG..." />
                                                    ...
                                                    <p className="text-xs text-gray-400 mt-1">Disp. estoque logístico ({req.equipmentType.split(' + ')[0].split(' (')[0].trim()}): <span className="font-bold">{Array.isArray(availableTags) ? availableTags.length : 0}</span></p>
                                                </>
                                            )}
                                        </div>
                                    </div>
