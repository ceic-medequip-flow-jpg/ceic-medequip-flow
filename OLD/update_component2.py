...
match = re.search(r'(const PendingRequestCard = \(\{ req.*?)(// View: Estoque Central \(Lista de equipamentos disponíveis\)\.\s*const InventoryView = )', content, re.MULTILINE|re.DOTALL)
if match:
...
\n'''
    ...
    content = content[:match.start(1)] + new_block + '\n        // View: Estoque Central (Lista de equipamentos disponíveis).\n        const InventoryView = ' + content[match.end(2):]
